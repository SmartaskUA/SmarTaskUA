...
# --- Gerar férias (30 dias por funcionário) ---
...
# Gera indivíduo: uma matriz (funcionários x dias) com 0 (folga) ou 1 (M) ou 2 (T)
def gerar_individuo():
    ...
    for i in range(nFuncionarios):
        ...
        while j < nDias:
            ...
            dias_trabalho = min(5, nDias - j)
            for k in range(dias_trabalho):
                if not ferias[i][j + k]:
                    individuo[i][j + k] = turno
            j += dias_trabalho
            if j < nDias:
                individuo[i][j] = 0
                j += 1
...
# Mutação: troca aleatória de turnos em dias que não sejam férias
def mutacao(individuo):
    i = random.randint(0, nFuncionarios - 1)
    d = random.randint(0, nDias - 1)
    if not ferias[i][d]:
        individuo[i][d] = random.choice([0] + preferencias[i])
    return individuo,

# Cruzamento: crossover entre dois funcionários
def cruzamento(ind1, ind2):
    i = random.randint(0, nFuncionarios - 1)
    ponto = random.randint(1, nDias - 2)
    ind1[i, ponto:], ind2[i, ponto:] = ind2[i, ponto:].copy(), ind1[i, ponto:].copy()
    return ind1, ind2
...
if __name__ == "__main__":
    ...
    print("Calendário exportado para calendario_deap.csv")
