...
def load_minimums_from_csv(mins_csv_file):
    ...
    Equipa A | 09:00-09:30 | -1 | 4 | 3 | ... 
    Equipa A | 09:30-10:00 | -1 | 4 | 4 | ...
    ...
    for _, row in df.iterrows():
        ...
        for idx, val in enumerate(row_values[:5]):  # Check first 5 columns
            ...
            if 'Equipa' in val_str or 'Team' in val_str:
                ...
                if match:
                    team = match.group(1)
            # Match 30-min format: 09:00-09:30 or 09-10 (legacy)
            elif re.match(r'^\d{2}:\d{2}-\d{2}:\d{2}$', val_str) or re.match(r'^\d{2}-\d{2}$', val_str):
                hour_range = val_str
        ...
        if team and hour_range and values_start_idx:
            ...
            # Format: "Team A,min,09:00-09:30,val1,val2,val3,..."
...
def extract_minimums_from_csv(file_path):
    ...
    and calculates the number of employees per 30-min slot, per day, and per team.

    Returns a dictionary in the format:
        {(day_number, '09:00-09:30', 'A'): 3, (day_number, '09:30-10:00', 'A'): 2, ...}
    """
    ...
    def parse_block(cell):
        """
        Parse work blocks with 30-min precision.
        Format: START-BREAK-END_TEAM where times can be:
        - Integer (e.g., 9, 12, 18) = hour exactly
        - Decimal .5 (e.g., 9.5, 14.5) = hour + 30 minutes
        
        Example: 9.5-14.5-18.5_A means:
        - Work: 09:30 to 14:30
        - Break: 14:30 to 15:30 (1 hour)
        - Work: 15:30 to 18:30
        """
        if not isinstance(cell, str):
            return None
        
        # Match format: 9.5-14.5-18.5_A or 9-14-18_A
        m = re.match(r"^\s*(\d{1,2}(?:\.\d)?)\s*[-_]\s*(\d{1,2}(?:\.\d)?)\s*[-_]\s*(\d{1,2}(?:\.\d)?)\s*[_\-]\s*([A-Za-z])\s*$", str(cell).strip())
        if not m:
            return None
        ...
        break_min = time_to_minutes(break_str)
        ...
        while current < break_min:
            ...
            slot = f"{hour:02d}:{minute:02d}-{next_hour:02d}:{next_minute:02d}"
            slots.append(slot)
            current += 30
    ...
    for day_col in day_cols:  # iterate over day columns
        ...
        for _, row in df.iterrows():  # iterate over rows
            ...
            if not parsed:
                continue
            slots, team = parsed  # unpack 30-min slots and work team

            for slot in slots: 
                key = (day_num, slot, team)  # create unique key
...
def compare_minimums(mins_required, mins_actual):
    ...
    Returns:
        Total of missed minimums (coverage gaps)
    """
    ...
    # Iterate through required minimums
...
def analyze(file, holidays, mins, employees, year=2025):
    ...
    - mins: CSV text of requirements per 30-min slot.
    ...
    # Re-order after adding missing days
    ...
    # Calculate coverage gaps and overstaff
    ...
    # Use key-based lookup for proper alignment
    ...
    # Calculate minimums gaps using the corrected function with dictionaries
...
def parse_requirements_hourly(requirements_text):
    ...
    for line in lines:
        ...
        # Detect pattern like "Equipa A,09:00-09:30"
        if len(parts) >= 3 and ("Equipa" in parts[1] or "Team" in parts[1]):
            team_code = parts[1].split()[-1].upper().strip()
            hour_range = parts[2]
            values = parts[3:]
            target = mins
        elif len(parts) >= 3 and (re.match(r'^\d{2}:\d{2}-\d{2}:\d{2}$', parts[1]) or re.match(r'^\d{2}-\d{2}$', parts[1])):
...
if __name__ == "__main__":
    print("="*80)
    print("LOCAL TESTING MODE - kpiVerification_HoursLocal_30min.py")
    print("="*80)

    # Configuration for local testing
    #CSV_FILE = "Horário_Por_Intervalos_1Hora_7hours_to_run.csv"
    CSV_FILE = "Heuristica_HH_Case1.csv"  # Your 30-min schedule file
