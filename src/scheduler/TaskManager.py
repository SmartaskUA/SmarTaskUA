...
from excel_export import export_schedule_to_excel, build_export_filename
...
class TaskManager:
    ...
    def run_task(self, task_id, title, algorithm_name="CSP Scheduling", vacations=None, minimuns=None, employees=None, maxTime=10, year=None, shifts=2, rules=None, hours=13, solver="CBC", problem_path=None):
        ...
        try:
            export_dir = os.environ.get("SCHEDULE_EXPORT_DIR", "/app/exports")
            filename = build_export_filename(algorithm_name, task_id)
            excel_export_path = export_schedule_to_excel(
                schedule_data, os.path.join(export_dir, filename)
            )
            print(f"[TaskManager] Schedule exported to Excel: {excel_export_path}")
        except Exception as e:
            print(f"[TaskManager] Excel export failed: {e}")

        result = {"schedule": schedule_data, "kpis": kpis, "sisqual_export": sisqual_export}
        if excel_export_path:
            result["excel_export_path"] = excel_export_path
        if extra_metadata:
