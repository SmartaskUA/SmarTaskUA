...
class CPHourScheduler:
    ...
    def _prepare_data(self):
        ...
        self.theta, self.beta = rows_to_req_dicts_Half_Hour(self.minimums)
        ...
        self.hours = [f"{h:04.1f}-{h+0.5:04.1f}" for h in range(9, 22)]
    ...
    def _define_vars(self):
        ...
        # cobertura insuficiente por dia, hora, equipa (O que falta)
        self.y = {}
        for d in range(self.num_days):
            for h in self.hours:
            ...
            for h in self.hours:
                for e in self.all_teams:
                    ...
                    if theta_val == -1:
                        ...
                        self.model.Add(covered + self.y[(d,h,e)] >= theta_val) # cobertura + insuficiência >= mínimo


        # 6. Descanso mínimo de 12h entre dias
        # Pre computa incompatibilidades, reduzindo número de constraints necessárias
        for i in self.I:
            for d in range(self.num_days - 1):
                for a1 in self.A:
                    for a2 in self.incompatible_blocks[a1]:
                        self.model.Add( # Caso a soma seja 2, significa que trabalha hoje e amanhã em blocos incompatíveis
