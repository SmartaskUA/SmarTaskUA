...
# -------------------------------------------------------
        ...
        for d in dias: 
            ...
            if d in self.closed_days:
                ...
                if flag:
                    print(f"[Heuristica] Day {d} is closed. Skipping.")
            ...
            for f in create_global_order:
                ...
                # Resetar sempre que o funcionário NÃO recebe bloco neste dia (exceto férias)
                ...
                if len(Block_Indexes) == 0:
                    ...
                    Worked_Sequential_Days[f] = 0
                    if flag:
                        print(f"[Heuristica] Day {d}, Emp {f}: No blocks left to assign, skipping.")
                    continue  # Nenhum bloco a atribuir

                if Worked_Sequential_Days[f] >= 5:
                    # Resetar o contador pois não trabalhou neste dia
                    Worked_Previous_Day[f] = None
                    Worked_Sequential_Days[f] = 0
                    if flag:
                        print(f"[Heuristica] Day {d}, Emp {f}: Skipping due to 5 days worked sequentially.")
