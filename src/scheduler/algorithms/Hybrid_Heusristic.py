...
class Heuristica:
    ...
    def evaluate_Day_Toshifts(self, day, mins, ideals):
        """
        ...
        for team_code in self.teams.keys():
            ...
            for shift in range(1, self.shifts + 1):
                ...
                optional.extend([shift] * max(0, ideal_count - min_count))
    ...
    def build_model(self, debug_daily_trace=False, debug_day_delay_seconds=10.0):
        ...
        for d in dias:
            daily_assignments = []
            dayunassigned = []  # Reset the list of unassigned employees for the new day
            ...
            Shifts_Table = self.evaluate_Day_Toshifts(d, mins, ideals)

            # Ordem de funcionarios pelo score (maior para menor)
            create_global_order = self.order_of_ranks(Pontuation)


            prev_shift_snapshot = {f: Worked_Previous_Day[f]    for f in funcionarios}
            prev_seq_snapshot   = {f: Worked_Sequential_Days[f] for f in funcionarios}
            ...
            # ================================================================
            # FASE 2 — preencher os IDEAIS
            # ================================================================
            
            n_ideais = 0
            
            for f in dayunassigned:
                ...
                for team_code, _ in teams_ideally_needed:
                    
                    if assigned:
                        break

                    next_shift = Shifts_Table[team_code]["optional"][0]

                    # CORRECÇÃO TRANSIÇÃO: usa o snapshot do turno anterior
                    # real, em vez de Worked_Previous_Day[f] que pode ser
                    # None por causa dos resets da fase 1.
                    prev_shift = prev_shift_snapshot[f]
                    ...
                    assigned_shift = Shifts_Table[team_code]["optional"].pop(0)
