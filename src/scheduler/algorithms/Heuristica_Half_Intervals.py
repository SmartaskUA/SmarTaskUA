...
class Heuristica:
    ...
    def _solve_block_cover_ilp(self, day, mins, team_code):
        ...
        # Use half-hour intervals (9.0, 9.5, 10.0, ..., 21.5)
        hours = drange_indexed_h(9, 22, 0.5)

        # Demanda real (mins já tem keys com half-hours)
        demand = {
...
# -------------------------------------------------------
        ...
        for d in dias: 
            ...
            # Minimo do dia
            # Extract minimums for this specific day
            # self.minimos keys are: (datetime, hour_str, team_code)
            # We need to build mins dict with keys: (hour_float, team_code)
            # CRITICAL: Expand hourly requirements to ALL half-hours in that hour
            # Example: '09-10' with requirement 3 means:
            #   - Need 3 people during 09:00-09:30 (h=9.0)
            #   - Need 3 people during 09:30-10:00 (h=9.5)
            mins = {}
            
            # First, collect all entries for this day
            day_entries = []
            for (date_key, hour_str, team_code), val in self.minimos.items():
                if date_key == d:
                    day_entries.append((hour_str, team_code, val))
            
            # Sort by hour_str to ensure consistent processing order
            day_entries.sort(key=lambda x: x[0])
            
            # Now process in sorted order
            for hour_str, team_code, val in day_entries:
                if ':' in hour_str:
                    # Format: '09:30-10:00' (already half-hour specific)
                    parts = hour_str.split('-')[0].split(':')
                    h_float = float(int(parts[0])) + (float(int(parts[1])) / 60.0)
                    # Round to 1 decimal to avoid float precision issues (9.5000000001 → 9.5)
                    h_float = round(h_float, 1)
                    mins[(h_float, team_code)] = val
                else:
                    # Format: '09-10' (hourly requirement)
                    # Expand to BOTH half-hours with SAME requirement
                    h_start = float(hour_str.split('-')[0])
                    h_end = float(hour_str.split('-')[1])
                    
                    # Generate half-hour intervals: 9.0, 9.5 for '09-10'
                    current_h = h_start
                    while current_h < h_end:
                        # Round to avoid floating point issues
                        h_rounded = round(current_h, 1)
                        mins[(h_rounded, team_code)] = val
                        current_h += 0.5
