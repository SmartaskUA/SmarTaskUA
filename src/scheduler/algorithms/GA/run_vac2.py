"""
run_vac2.py — GA runs for 2-shift 2-team scenario with Vacation Scenarios 2 and 3.

Vacation Scenario 2: Random Two-Block Distribution (2×15 days, randomly placed).
Vacation Scenario 3: same format, different random template.
Data directories: SMARTASK_SIMPLE_2025_VAC2, SMARTASK_SIMPLE_2025_VAC3

Same GA config as run_final.py (Config C: nbts + demand_guided, pop=200, patience=100).

Usage:
    python run_vac2.py

Output:
    results_vac2/SMARTASK_SIMPLE_2025_VAC2/ga_results.csv
    results_vac2/SMARTASK_SIMPLE_2025_VAC3/ga_results.csv
    (+ convergence/ and schedules/ subdirs for each)
"""
...
SCENARIOS = [
    {"data_dir": "SMARTASK_SIMPLE_2025_VAC2", "n_runs": 10, "early_stop_patience": 100, "n_workers": None},
    {"data_dir": "SMARTASK_SIMPLE_2025_VAC3", "n_runs": 10, "early_stop_patience": 100, "n_workers": None},
    {"data_dir": "SMARTASK_SIMPLE_2025_VAC4", "n_runs": 10, "early_stop_patience": 100, "n_workers": None},
]
...
def run_scenario(data_dir, n_runs, patience, n_workers):
    ...
    output_dir = os.path.join("results_vac2", data_dir)
    ...
    print(f"\n{'='*56}")
    print(f"  {data_dir}  (Vacation Scenario 2, patience={patience}, runs={n_runs})")
    print(f"{'='*56}")
