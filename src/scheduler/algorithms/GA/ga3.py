"""
ga3.py — Shift scheduling GA for 3-shift scenarios (M, T, N).
...
def cx_nbts(ind1, ind2, n_emp, n_days, problem_data):
    ...
    for i in range(n_emp):
        ...
        if s1 > s2:
            ...
            row = arr1[i] if random.random() < 0.5 else arr2[i]
        child1[i] = row
    ...
    for i in range(n_emp - 1, -1, -1):
        ...
        if s2 > s1:
            ...
            row = arr2[i] if random.random() < 0.5 else arr1[i]
        child2[i] = row
...
def main():
    print("=" * 55)
    print("  SmarTask — Shift Scheduling GA (3 shifts: M, T, N)")
    print("=" * 55)
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig("ga3_fitness.png", dpi=120)
    print("Plot saved → ga3_fitness.png")
    plt.show()
