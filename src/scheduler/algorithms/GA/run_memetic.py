"""
run_memetic.py — Memetic GA validation runs for 2-shift scenarios.

Configuration: nbts + demand_guided + one-pass local search on all offspring.
Saves results to results_memetic/<scenario>/ for comparison with run_final.py.

Usage:
    python run_memetic.py

Output:
    results_memetic/<scenario>/ma_results.csv
    results_memetic/<scenario>/convergence/run*.npy
    results_memetic/<scenario>/schedules/run*.npy
"""
...
from memetic_ga import run_memetic_ga
...
def run_scenario(data_dir, n_runs, patience, n_workers):
    ...
    csv_path   = os.path.join(output_dir, "ma_results.csv")
    done_runs  = set()
    write_mode = "w"
    ...
    for run_idx in range(1, n_runs + 1):
        ...
        t0 = time.time()
        best_ind, best_fitness, logbook, stopped_at = run_memetic_ga(problem_data, params)
