"""
run_postprocess3.py — Post-processing local search on saved 3-shift GA schedules.

Applies day-swap and cyclic-exchange local search to each best schedule
produced by run_3shifts.py. Reads from results_3shifts/<scenario>/schedules/
and writes results to results_3shifts/<scenario>/pp_results.csv.

Run AFTER run_3shifts.py has completed for the desired scenarios.
...
from problem3 import load_problem, _compute_penalties, local_search_ideal, local_search_cyclic

DATA_DIR = os.path.join(os.path.dirname(__file__), "../../../../data/problems")

SCENARIOS = [
    "SMARTASK_3SHIFTS_2TEAMS_2025",
    "SMARTASK_3SHIFTS_4TEAMS_2025",
    "SMARTASK_3SHIFTS_8TEAMS_2025",
    "SMARTASK_3SHIFTS_16TEAMS_2025",
    "SMARTASK_3SHIFTS_32TEAMS_2025",
]
...
def postprocess_scenario(data_dir):
    base_dir  = os.path.join("results_3shifts", data_dir)
    sched_dir = os.path.join(base_dir, "schedules")

    if not os.path.isdir(sched_dir):
        print(f"  No schedules found for {data_dir} — run run_3shifts.py first.")
        return
    ...
    problem_data = load_problem(os.path.join(DATA_DIR, data_dir))
