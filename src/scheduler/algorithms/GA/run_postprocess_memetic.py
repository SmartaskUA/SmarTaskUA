"""
run_postprocess_memetic.py — Post-processing on memetic (ls_mode=best) schedules.

Reads from results_memetic_best/<scenario>/schedules/ and writes
pp_results.csv to the same folder. Logic identical to run_postprocess.py.

Usage:
    python run_postprocess_memetic.py
"""
...
SCENARIOS = [
    "SMARTASK_SIMPLE_2025",
    "SMARTASK_4TEAMS_2025",
    "SMARTASK_8TEAMS_2025",
]
...
def postprocess_scenario(data_dir):
    base_dir  = os.path.join("results_memetic_best", data_dir)
    sched_dir = os.path.join(base_dir, "schedules")

    if not os.path.isdir(sched_dir):
        print(f"  No schedules found for {data_dir} — run run_memetic.py first.")
        return

    print(f"\n{'='*56}")
    print(f"  {data_dir} — post-processing (memetic best)")
    print(f"{'='*56}")
