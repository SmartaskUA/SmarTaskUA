...
class SisqualProblem5CSP:
    ...
    def build_model(self):
        ...
        # days. Optional before-context days are fixed history, so they only
        # contribute constants to windows crossing the target-period boundary.
        for employee in self.employees:
            employee_id = employee["id"]
            for start in range(0, len(self.constraint_days) - 5):
                ...
                # Target days remain normal CP-SAT workday variables.
                fixed_workdays = sum(
                ...
                model.Add(
                    fixed_workdays + sum(target_terms) <= 5
                )
            ...
            for day, next_day in zip(self.constraint_days, self.constraint_days[1:]):
                ...
                for next_assignment in self.assignments[(employee_id, next_day)]:
                    ...
                    if rest_hours < self.min_rest_hours:
                        model.Add(self.x[(employee_id, next_day, next_assignment.key)] == 0)
...
def solve(
    ...
    **kwargs,
    ...
    status_name = scheduler.solution_status()
