...
class Hybrid_Heuristic_Sisqual:
    def __init__(self,
        ...
        self.day_off_swap = {}
        self.d_bar_windows = []
        self.unresolved_5in6_violations = []
        ...
        print(f"Objective Weights: {self.objective1_weight}, {self.objective2_weight}, {self.objective3_weight}")
        print(f"Variable Days Off Active: {self.variable_days_off_active}")
        print(f"Min Rest Hours: {self.min_rest_hours}")
        print(f"Max Time Seconds: {self.max_time_seconds}")

    def Pontuation(self, e, Previous_Assignment, Next_Assignment):

        return

    def ConsecutiveChecker(self, employee_id, day):
    ...
    def Order(self):
        ...
        for employee in ordered_employees:
            employee_skills = employee["assignable_skills"]
            employee_count = len(employee_skills)
            rarity_profile = tuple(
                sorted(skill_employee_count.get(skill, 0) for skill in employee_skills)
            )
            # print(
            #     f"[Heuristica] Employee {employee['id']} | Assignable Skills: {employee_skills} | "
            #     f"Count: {employee_count} | Rarity: {rarity_profile}"
            # )
    ...
    def _best_skill_for_slot(self, employee: Dict, day: str, slot_idx: int) -> Tuple[Optional[str], int]:
        """Among the employee's assignable skills, return the (skill, need)
        pair with the highest remaining shortage for this (day, slot).

        Ties are broken by scarcity: the skill with fewer capable employees
        overall (`self.skill_employee_count`) wins, since it's harder to cover
        later if this employee doesn't take it now. A further tie (equal need
        and equal headcount) falls back to the order in `assignable_skills`,
        just to stay deterministic.
        """
        best_skill = None
        best_need = -1
        best_count = None
        for skill in employee["assignable_skills"]:
            ...
            # print(f"[Heuristica_Best_Skill] Employee {employee['id']} on {day}, slot {slot_idx}: skill {skill} has remaining need = {remaining}, capable employees = {count}")

            if best_skill is None:
                better = True
            elif remaining > best_need:
                better = True
            elif remaining == best_need and count < best_count:
                better = True
            else:
                better = False
        ...
        if best_skill is None:
            return None, 0
        # print(f"[Heuristica_Best_Skill] Employee {employee['id']} on {day}, slot {slot_idx}: best skill = {best_skill} with need = {best_need} (capable={best_count})")
        return best_skill, max(best_need, 0)

    def _score_block(self, employee: Dict, day: str, block: Assignment) -> int:
        """Sum, over every slot in the block, the shortage that the
        employee's best-fitting skill would cover. Blocks that relieve more
        outstanding minimums score higher.
        """
        score = 0
        for slot_idx in block.slot_indices:
            _, need_here = self._best_skill_for_slot(employee, day, slot_idx)
            score += need_here
            # print(f"[Heuristica_Score_Block] Employee {employee['id']} on {day}, slot {slot_idx}: best skill need = {need_here}, cumulative score = {score}")
        return score

    def _neighbor_end_min(self, employee_id: str, day: str) -> Optional[int]:
        """End time (minutes) of the block already fixed for the day right
        before `day`, or None if unknown (not processed yet / no exact
        context info)."""
    ...
    def _neighbor_start_min(self, employee_id: str, day: str) -> Optional[int]:
        """Start time (minutes) of the block already fixed for the day right
        after `day`, or None if unknown (not processed yet / no context)."""
        current_date = self.date_by_day[day]
    ...
    def _rest_ok(self, employee_id: str, day: str, block: Assignment) -> bool:
        """Validate the 11h (config: `self.min_rest_hours`) minimum rest
        against whatever is already fixed for the previous and next day for
        this employee. Boundaries with unknown exact times are skipped
    ...
    def _apply_block(self, employee: Dict, day: str, block: Assignment) -> None:
        """Fix `block` for (employee, day): pick the best skill per slot,
        record it, and decrement `self.need` accordingly."""
        employee_id = employee["id"]
        for slot_idx in block.slot_indices:
            # print(f"[Heuristica_Apply_Block] Assigning {employee_id} on {day}, slot {slot_idx}")
            skill, _ = self._best_skill_for_slot(employee, day, slot_idx)
            # print(f"[Heuristica_Apply_Block] Selected skill {skill} for {employee_id} on {day}, slot {slot_idx}")
            if skill is None:
                continue
            self.slot_skill[(employee_id, day, slot_idx)] = skill
            key = (day, slot_idx, skill)

            # Atualiza a necessidade restante para o slot e skill escolhidos
            if key in self.need:
                self.need[key] = max(0, self.need[key] - 1)

            # print(f"[Heuristica_Apply_Block] Updated need for {key}: {self.need.get(key, 0)}")

        self.employee_day_block[(employee_id, day)] = block

    def assign_employee_day(self, employee: Dict, day: str) -> bool:
        """Try to fix a block for (employee, day), trying candidates from
        best to worst coverage score, falling back to the next-best whenever
        the current one breaks the rest-hours rule. Returns True on success.
        """
        employee_id = employee["id"]
        candidates = self.assignments.get((employee_id, day), [])
        # print(f"[Heuristica_Assign_Employee_day] Assigning {employee_id} on {day}: {(candidates)} candidates")

        if not candidates:
            # Fixed non-working day (VAC/NOT/Med/FDO/closed/preferred day off)
            self.employee_day_block[(employee_id, day)] = None
            return False

        scored = [(self._score_block(employee, day, block), random.random(), block) for block in candidates]
        # Best coverage first; random.random() gives a stable-but-random tiebreak.
        scored.sort(key=lambda item: (-item[0], item[1]))

        # print(f"[Heuristica_Assign_Employee_day] Scored candidates for {employee_id} on {day}:")
        # for score, _, block in scored:
        #     print(f"  - {block.label}: score={score}")

        for _, _, block in scored:
            if self._rest_ok(employee_id, day, block):
                # print(f"[Heuristica_Assign_Employee_day] Assigning {employee_id} on {day}: Chose block {block.label}, rest verified")
                self._apply_block(employee, day, block)
                return True

        # Every candidate violates the rest-hours rule against an already
        # fixed neighbor -> no feasible assignment today.
        self.employee_day_block[(employee_id, day)] = None
    ...
    def _day_bottleneck_score(self, day: str) -> float:
        """Quanto maior, mais 'apertado' é o dia: rácio entre procura ainda
        por cobrir e nº de empregados capazes de a cobrir nesse dia,
        somado por skill. Ao contrário da procura total, isto penaliza dias
        com pouca folga de mão-de-obra mesmo que a procura absoluta seja
        modesta.
        """
    ...
    def _compute_day_order(self, day_order_mode: int = 1) -> List[str]:
        """Decide a ordem pela qual os dias são processados em Minimuns().

        day_order_mode:
            1 -> aleatório (comportamento original)
            2 -> por procura total do dia (soma de self.alpha), descendente,
                 com desempate aleatório para manter alguma diversidade entre
                 restarts
            3 -> por 'aperto' (bottleneck): rácio procura/disponibilidade por
                 skill, somado por dia, descendente, com desempate aleatório
        """
        if day_order_mode == 1:
        ...
        raise ValueError(f"day_order_mode inválido: {day_order_mode!r} (usa 1, 2 ou 3)")
    ...
    def Minimuns(self, day_order_mode):
        ...
        List_Employees = [e["id"] for e in self.employees]
        Pontuation = {e: 0 for e in List_Employees}
        Daily_Needs = {}
        for skill in self.skills:
            Daily_Needs[skill] = {}
        ...
        for level in self.levels:
            # print(f"\n[Heuristica] Level {level}")
            for emp_id, skill, name in self.employees_by_level.get(level, []):
                # print(f" - {emp_id} | {skill} | {name}")
                pass

        ordered_employee_ids = self.Order()
        employees_by_id = {e["id"]: e for e in self.employees}
        ...
        for d in random_days:
            ...
            # if count == 5:
            #     time.sleep(1000)  # Optional: slow down for debugging output readability

            # Random tie-break for "empate de urgência": preserve the base
            # priority from `self.Order()` but randomize only among employees
            # that share the same priority key (i.e., true ties). The previous
            # approach shuffled the whole list then sorted it back, cancelling
            # the shuffle entirely.
            # Build a priority key consistent with `Order()` (without the
            # deterministic `employee['id']` final tiebreak) and shuffle per
            # bucket.
            skill_employee_count = {
                skill: sum(1 for employee in self.employees if skill in employee["assignable_skills"])
                for skill in self.skills
    ...
    def export_csv(self, filename="schedule_weighted.csv"):
        raise NotImplementedError(
            "export_csv depende de build_output_rows, que só pode ser "
            "montado depois das Fases B e C atribuírem blocos e skills."
        )


    def build_output_rows(self) -> List[List[str]]:
        """Exporta o resultado da heurística gulosa no mesmo formato CSV que o
        modelo exato (ILP) produzia, para que o programa a jusante não precise
        de saber qual dos dois motores gerou o horário.
    
        Ao contrário da versão ILP, aqui não há variáveis PuLP para consultar:
        o bloco escolhido para (empregado, dia) já está em
        `self.employee_day_block`, e a competência escolhida por slot já está
        em `self.slot_skill`. A função apenas lê esse estado e formata as
        linhas.
        """
            ...
            for day in self.days:
                ...
                if chosen is None:
                    if mode == "preferred_day_off":
                        ...
                        # Fallback defensivo: dia de trabalho esperado mas sem
                        # bloco e sem estar marcado como unassigned — não
                        # deveria acontecer com a lógica atual do Minimuns().
    ...
    def compute_true_shortage(self) -> int:
        """Compute total shortage using the same logic as the KPI evaluator.

        Builds the CSV-style rows via `build_output_rows()` and calls
        `KpiEvaluator_Sisqual.compute_Total_Shortage()` to get the canonical
        shortage value reported by the KPIs. Stores the value on
        `self.total_shortage` and returns it as an int.
        """
...
def solve(problem_path=None, maxTime=None, restarts=10, day_order_mode=None, **kwargs):
    ...
    # Log all results: list of dicts with restart_num, day_order_mode, kpi_shortage, internal_shortage
    all_results = []
    ...
    for i in range(restarts):
        ...
        for current_mode in day_order_mode_list:
            ...
            if best_shortage is None or kpi_shortage < best_shortage:
                best_shortage = kpi_shortage
                best_rows = scheduler.build_output_rows()

    results_log_file = kwargs.get("results_log_file") or kwargs.get("log_file")

    # Write results to CSV file
    _write_results_log(problem_path, all_results, best_shortage, results_log_file=results_log_file)
    
    print(f"[Heuristica] Best total shortage after {restarts} restarts: {best_shortage}")

    return best_rows


def _write_results_log(problem_path: str, all_results: list, best_shortage: int, results_log_file: str | None = None):
    """Write all restart results to a CSV log file."""
    try:
        problem_path = Path(problem_path)
        base_dir = problem_path.parent
        if results_log_file:
            log_file = Path(results_log_file).expanduser()
            if not log_file.is_absolute():
                log_file = Path.cwd() / log_file
        else:
            if os.access(base_dir, os.W_OK):
                log_file = base_dir / "heuristic_results_no_levels.csv"
            else:
                log_file = Path.cwd() / "heuristic_results_no_levels.csv"

        try:
            log_file.parent.mkdir(parents=True, exist_ok=True)
            with log_file.open('w', newline='', encoding='utf-8') as f:
                writer = csv.DictWriter(f, fieldnames=['restart_num', 'day_order_mode', 'kpi_shortage'])
                writer.writeheader()
                for row in all_results:
                    writer.writerow(row)
        except OSError:
            fallback_log_file = Path.cwd() / "heuristic_results_no_levels.csv"
            fallback_log_file.parent.mkdir(parents=True, exist_ok=True)
            with fallback_log_file.open('w', newline='', encoding='utf-8') as f:
                writer = csv.DictWriter(f, fieldnames=['restart_num', 'day_order_mode', 'kpi_shortage'])
                writer.writeheader()
                for row in all_results:
                    writer.writerow(row)
        ...
        print(f"[Heuristica] Best shortage found: {best_shortage}")
    except Exception as ex:
