"""
------------------------------------------------------------
MINIMUMS SCALER GENERATOR (3 SHIFTS)
------------------------------------------------------------
Goal:
  ...
  configuration with 3 shifts (2 teams, 15 employees).

What this script does:
  1. Reads the base minimuns3shifts.csv (staffing per day per team/shift)
  ...
  6. Saves new minimuns_3shifts_xxteams_xxemp.csv files

Notes:
  - Targets keep the same team counts as the 2-shift version.
  - Total employees per scenario are doubled to cover the extra shift.
------------------------------------------------------------
"""
...
# === CONFIGURATION ===
# Base file path
base_file = "data/base_files/minimuns3shifts.csv"
...
targets = [
    (teams, int(round(employees * employee_multiplier)))
    for teams, employees in base_targets
]
...
# Extract only Equipa A's pattern:
#   (Minimo/Ideal x Morning/Evening/Night)
pattern_df = df_base[df_base["Equipa"].str.contains("A", case=False, na=False)].reset_index(drop=True)
shift_count = df_base["Turno"].nunique()
pattern_df = pattern_df.head(shift_count * 2)
...
for teams, employees in targets:
    ...
    out_file = f"data/minimunsData/minimuns_3shifts_{teams}teams_{employees}emp.csv"
