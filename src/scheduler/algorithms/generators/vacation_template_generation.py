...
# =========================================================
def generate_case3(employee_count, days_per_year=365, seed=1337):
    """
    Case 3 – Weekend-Focused Short Breaks
    -------------------------------------
    Each employee takes multiple (4–6-day) vacations spread
    across the year, focusing on weekends (Fri–Mon overlap).

    Total ≈30 vacation days per employee per year.
    """
    rng = np.random.default_rng(seed)
    vacation_data, vacation_stats = {}, {}

    # Weekend-bias weights (Mon=0 … Sun=6)
    # People usually start vacation Thu/Fri/Sat
    weekday_bias = np.array([0.8, 0.8, 1.0, 1.5, 2.0, 1.7, 1.3])
    weekday_bias /= weekday_bias.sum()

    def _biased_start_day():
        """Choose a start day with bias toward weekends."""
        # Pick random day but weighted by weekday bias
        start = rng.integers(1, days_per_year + 1)
        weekday = (start - 1) % 7  # 0=Mon
        if rng.random() < weekday_bias[weekday] / weekday_bias.max():
            return start
        # Otherwise nudge toward nearest weekend
        shift = rng.choice([-1, 1, 2])
        return _wrap_day(start + shift, days_per_year)
    ...
    for emp_id in range(1, employee_count + 1):
        ...
        while total_days < 30:
            # Space breaks ~30–40 days apart
            start = _biased_start_day()
            while abs(start - last_start) < 25:
                start = _biased_start_day()

            length = rng.integers(4, 7)  # 4–6 days
            block = _block_days(start, length, days_per_year)

            # Add only unused days
            new_days = [d for d in block if d not in days]
            days.update(new_days)
            total_days = len(days)
            last_start = start

        # If overshoot slightly (e.g., 31–32 days), trim random ones
        if total_days > 30:
            extra = total_days - 30
            for d in rng.choice(list(days), extra, replace=False):
                days.remove(d)
