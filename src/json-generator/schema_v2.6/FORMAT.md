# JSON + CSV Hybrid Format Reference (v2.6)
...
|-------|------|----------|-------------|
| `schemaVersion` | string | ✓ | Must be `"2.6"` |
...
**Purpose:** Daily coverage requirements - tells algorithm HOW MANY people are needed each day. v2.6 adds optional work period time overrides.
...
| `workPeriod` | string | Work period code (must match workPeriods[].code in JSON) | `M`, `T`, `N` |
...
2. Work period codes must exist in `demand.workPeriods[].code`
...
## Complete Example Structure (v2.6)
...
| `scheduleInput` CSV "A" value | Available | **Changed:** Auto-allocate | Same as v2.2 | Same as v2.2 |
| `scheduleInput` CSV numeric values | Invalid | **New:** Valid (1-16) | Same as v2.2 | Same as v2.2 |
| CSV file count | 2 files | 2 files | 3 files | **2 files** |
...
2. **Remove operatingHours section** from JSON entirely
3. **Add open/close columns to demand.csv** (optional columns)
4. **Delete operating_hours.csv** file

### Example Migration:

**Before (v2.5) - Two files:**
...
{
  "schemaVersion": "2.6"
  // operatingHours section removed
}
```
...
`operating_hours.csv` - **DELETE THIS FILE**
...
## Validation Checklist (v2.6)

### JSON Validation
- [ ] Validates against schema.json
- [ ] `schemaVersion` is `"2.6"`
...
- [ ] Work periods have `timeRange` defined (used as defaults)
...
- [ ] demand.csv work period codes match JSON `demand.workPeriods[].code`
- [ ] demand.csv team codes match JSON `organizationalUnits.teams[].code`
