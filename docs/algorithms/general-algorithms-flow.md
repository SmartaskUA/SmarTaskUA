...
4. The API publishes a RabbitMQ payload.
   Code: `src/api/src/main/java/smartask/api/event/RabbitMqProducer.java:74`
5. The Python scheduler consumes the payload and loads the referenced templates from MongoDB.
   Code: `src/scheduler/RabbitMQClient.py:139`
...
3. It builds template names and rows for vacations and minimums.
   Code: `src/api/src/main/java/smartask/api/services/ProblemService.java:606`
...
3. It writes the resulting minimums rows into the `ReferenceTemplate` collection.
   Code: `src/api/src/main/java/smartask/api/services/ProblemService.java:630`
...
2. The scheduler reads `rules` from the message.
   Code: `src/scheduler/RabbitMQClient.py:143`
3. `TaskManager` passes that map as `constraints` only for the general algorithms.
   Code: `src/scheduler/TaskManager.py:68`
...
4. The parser supports both hard and soft rules.
   ...
   Code: `src/scheduler/algorithms/general/constraints.py:163`
...
2. They normalize year and infer the number of shifts from the minimums data.
   Code: `src/scheduler/algorithms/general/ilp_general.py:575`
   Code: `src/scheduler/algorithms/general/csp_general.py:35`
3. They convert vacations and minimums rows into dictionaries keyed by `(day, shift, team)`.
   Code: `src/scheduler/algorithms/utils.py:72`
   Code: `src/scheduler/algorithms/utils.py:177`
...
1. Plan values are wired into the scheduler instance at construction time.
   Code: `src/scheduler/algorithms/general/ilp_general.py:580`
...
3. Coverage weights affect the objective directly.
   Code: `src/scheduler/algorithms/general/ilp_general.py:395`
...
1. If coverage is soft, it introduces unmet-demand variables.
   Code: `src/scheduler/algorithms/general/csp_general.py:149`
   Code: `src/scheduler/algorithms/general/csp_general.py:165`
2. Coverage weights become coefficients in the objective.
   Code: `src/scheduler/algorithms/general/csp_general.py:186`
   Code: `src/scheduler/algorithms/general/csp_general.py:191`
...
2. `RabbitMqProducer.requestScheduleMessage(...)` payload construction.
   Code: `src/api/src/main/java/smartask/api/event/RabbitMqProducer.java:74`
3. `RabbitMQClient.callback(...)` and `handle_task_processing(...)`.
   Code: `src/scheduler/RabbitMQClient.py:65`
   Code: `src/scheduler/RabbitMQClient.py:183`
