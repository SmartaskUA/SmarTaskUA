...
| `date,workPeriod,team,minimum,ideal,estimated` | `date,workPeriod,team,minimum,empiric,maximum` (ascending; `0` = unset) | see the callout below — this one bites |
...
| `employees.simple[]` / `competency[]` | one `employees.list[]` (`model` selects semantics) | one list |
| `teams[]` + `contractType` + `contractPeriods[]`, teams **static** | `contractAssignments[]` + `teamAssignments[]`, both date-ranged `{start, end|null}` | teams change over time too; static membership → one open-ended entry |
| `priorityHierarchy[] {rank, team, level:"N>=2"}` | `demand.priorityOrder[] {order, team, level?}`, first match wins | `"N>=2"` parsed nowhere; you author no weights — the solver derives `p_sl` from position |
...
6. Rename `priorityHierarchy` → `priorityOrder`, `rank` → `order`; drop free-form `level` strings.
