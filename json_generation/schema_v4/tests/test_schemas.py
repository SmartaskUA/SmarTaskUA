...
from helpers import C2, load


def test_both_schemas_compile(schemas):
    assert set(schemas) == {"schema-v4-input.json", "schema-v4-result.json"}
...
def test_each_schema_validates_its_real_instance(schemas):
    ...
    Draft202012Validator(schemas["schema-v4-input.json"]).validate(
...
def test_root_is_open_but_nested_objects_are_closed(schemas):
    ...
    from jsonschema import Draft202012Validator
    schema = schemas["schema-v4-input.json"]
...
def test_dimensions_is_required(schemas):
    ...
    demand = schemas["schema-v4-input.json"]["$defs"]["demand"]
...
def test_soft_constraints_stay_unconstrained(schemas):
    ...
    soft = schemas["schema-v4-input.json"]["$defs"]["constraints"]["properties"]["soft"]
